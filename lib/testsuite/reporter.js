...
// Reporter holds information about a given test suite and displays/reports those results to consumer
// see TestSuite where this class gets constructed.
class Reporter {
  /**
   *
   * @param {Array} tests
  ...
  registerTestError(err) {
    ...
    err.detailedLogging = err.detailedLogging || Utils.isUndefined(err.detailedLogging);
    if (this.shouldIncrementTotalCount(err) && err.detailedLogging) {
  ...
  // Called by test suite to print out results on finishing.
  printTestResult() {
    let ok = false;
